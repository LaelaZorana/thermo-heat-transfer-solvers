...
thermohx is the core of an undergraduate thermo and heat transfer sequence turned into code you can run, with the power and refrigeration cycles working off real fluid data through CoolProp. It keeps its references close, because a correlation you can't trace back to a textbook value turns into a liability the first time someone asks where the number came from. Extended surfaces, heat exchanger sizing and rating, and transient conduction each get their own row in the table below. Push a convection correlation outside the range it was fitted on and it'll warn you, and if an input makes no physical sense it raises rather than handing back something that merely looks reasonable. Every module carries at least one textbook validation case in its docstrings, which the test suite checks. Nothing here is validated by eye.
